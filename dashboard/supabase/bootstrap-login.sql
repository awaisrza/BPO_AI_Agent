-- Complete login bootstrap — run once in Supabase SQL Editor
...
begin
  ...
  if oid is not null then
    if exists (select 1 from organizations where id = oid) then
      return oid;
    end if;
  ...
  values (coalesce(uname->>'org_name', 'My Call Center'))
