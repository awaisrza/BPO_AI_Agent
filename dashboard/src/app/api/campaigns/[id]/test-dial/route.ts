...
import { getOrganizationForCampaign } from "@/lib/auth";
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const org = await getOrganizationForCampaign(id);
  const emptyOrg = { vicidial_url: null, vicidial_user: null, vicidial_pass: null };
  const creds = resolveVicidialCreds(org ?? emptyOrg);
  if (!creds.configured) {
    return NextResponse.json(
  ...
  let gpuMessage = "";
  if (startCampaign) {
    await setCampaignRunningStatus(supabase, id, "running");
    try {
      ...
      gpuMessage =
        ...
        " Campaign status is running — if the supervisor poll loop is active on the GPU, the worker should still start.";
    }
  }
