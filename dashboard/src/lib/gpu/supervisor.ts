...
export async function syncGpuSupervisor(campaignId: string, action: "start" | "stop") {
  ...
  if (!baseUrl) {
    return { ok: true, configured: false, message: "GPU supervisor URL not set — workers rely on poll loop only." };
  }

  const path = action === "start" ? "/sync" : "/stop";
  const query = action === "start" ? `?campaign_id=${encodeURIComponent(campaignId)}` : `?campaign_id=${encodeURIComponent(campaignId)}`;
